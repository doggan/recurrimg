...
function downloadImages(imageUrls, outputDir, onFinished) {
    var work = [];
...
function main() {
    ...
    doRecursiveImageSearch(count, startImageUrl, function(err, resultImageUrls) {
        ...
        downloadImages(resultImageUrls, dir, function(err) {
            if (err) {
                console.error('Image download failure.');
                return;
            }

            var gifMaker = require('./gif_maker');
            var gifPath = './result.gif';
            gifMaker.makeGif(dir, gifPath, targetGifSize);
        });
    });
}
